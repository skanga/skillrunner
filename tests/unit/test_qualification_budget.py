...
def test_authorized_qualification_ceiling_accepts_25_but_rejects_more(tmp_path):
    path = tmp_path / "spending.json"
    seed(path)
    ...
    state["budget_usd"] = "25.01"
    path.write_text(json.dumps(state))
    with (
        ledger_api().SpendingLedger(path) as ledger,
        pytest.raises(ValueError, match=r"authorized \$25 ceiling"),
    ):
        ledger.require_authorized_ceiling()
